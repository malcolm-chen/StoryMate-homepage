...
const ReadChatPage = () => {
    const user = 'Emma'
    const [title, setTitle] = useState('Why Frogs are Wet?');
    ...
    const penguin = '/imgs/penguin1.svg';
    ...
    useEffect(() => {
        const loadDictionary = async () => {
            try {
                ...
                const response = await fetch(`/${title}/${title}_knowledge_dict.json`);
        ...
        const loadStory = async () => {
            try {
                ...
                const response = await fetch(`/${title}/${title}_sentence_split.json`);
                ...
                const loadedPages = Array.from({ length: storyText.length }, (_, index) => ({
                    image: `/${title}/pages/page${index}.png`,
    ...
    async function getInstruction4Guiding() {
        ...
        **Story Information**:
        ...
        - First Question: ${knowledgeRef.current[currentPageRef.current]?.first_question_set[0]}
                        ...
                        {[...chatHistoryRef.current[currentPageRef.current], ...currentPageChatHistory].filter(msg => msg.type === 'message').map((msg, index) => (
                            ...
                            <Box key={index} id={msg.role === 'user' ? 'user-msg' : 'chatbot-msg'}>
                                {msg.role === 'user' ? (
                                    ...
                                    <Box id="chatbot-chat">
                                        <Image id='chatbot-avatar' src='/imgs/penguin.svg'></Image>
